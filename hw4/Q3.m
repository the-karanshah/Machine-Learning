...
function Q3_4_1()
    fprintf("Question 3.4.1");
    [trD, trLb, ~, ~, ~, ~, ~, ~] = HW4_Utils.getPosAndRandomNeg();
    C = 2;

    [w, b, ~, ~, ~] = solveSVM(trD, trLb, C);

    HW4_Utils.genRsltFile(w, b, 'val', 'q341');
    [ap, ~, ~] = HW4_Utils.cmpAP('q341', 'val');
    fprintf('AP : %d', ap(end));
end
...
function Q3_4_3()
    ...
    [trD, trLb, ~, ~, ~, ~, startNegDtrain, ~] = HW4_Utils.getPosAndRandomNeg();
    C = 2;
