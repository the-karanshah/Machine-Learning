...
% Last modified: 16-Oct-2018    
    ...
    methods (Static)
        function acc = main()
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % 3.4.3
            fprintf('Question 3.4.3');
            C = [0.1, 1, 10, 30, 65, 100, 200];
            gamma = [0.1, 0.5, 1.1, 1.4, 2.0];

            for i = 1:length(C)
                for j = 1:length(gamma)
                    config = sprintf('-c %d -g %d -v 5 -q', C(i), gamma(j));
                    %config = sprintf('-v 5');
                    acc = svmtrain(trLbs, trainD, config);
                    fprintf('C = %d, gamma = %d\n', C(i), gamma(j));
                    fprintf('****\n');
                end
                fprintf('**********\n');
        ...
        function bowCs = learnDictionary(scales, normH, normW)
            ...
            % load train ids
            trIds = ml_load('dataset/bigbangtheory_v3/train.mat', 'imIds'); 
            ...
            for i=1:length(trIds)
                ...
                im = imread(sprintf('dataset/bigbangtheory_v3/%06d.jpg', trIds(i)));
        ...
        function D = cmpFeatVecs(imIds, scales, normH, normW, bowCs)
            ...
            for i=1:n
                ...
                im = imread(sprintf('dataset/bigbangtheory_v3/%06d.jpg', imIds(i)));                                
